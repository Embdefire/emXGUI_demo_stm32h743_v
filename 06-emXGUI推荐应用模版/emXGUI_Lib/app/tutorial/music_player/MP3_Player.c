...
{
	{SAI_I2S_STANDARD, SAI_PROTOCOL_DATASIZE_16BIT, SAI_AUDIO_FREQUENCY_8K},
	{SAI_I2S_STANDARD, SAI_PROTOCOL_DATASIZE_16BIT, SAI_AUDIO_FREQUENCY_16K},
	{SAI_I2S_STANDARD, SAI_PROTOCOL_DATASIZE_16BIT, SAI_AUDIO_FREQUENCY_22K},
	{SAI_I2S_STANDARD, SAI_PROTOCOL_DATASIZE_16BIT, SAI_AUDIO_FREQUENCY_44K},
	{SAI_I2S_STANDARD, SAI_PROTOCOL_DATASIZE_16BIT, SAI_AUDIO_FREQUENCY_96K},
	{SAI_I2S_STANDARD, SAI_PROTOCOL_DATASIZE_16BIT, SAI_AUDIO_FREQUENCY_192K},
};
...
void mp3PlayerDemo(const char *mp3file)
{
   ...
   WCHAR wbuf[128];//保存文本数组
	...
	mp3player.ucVolume = 35;//设置 WM8978的音量值
   int ooo = 0;
	...
	SAIA_TX_DMA_Init((uint16_t *)outbuffer[0],(uint16_t *)outbuffer[1],MP3BUFFER_SIZE);	
  ...
  SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC5), wbuf);    
	read_ptr=inputbuf;
	bytes_left=bw;
...
//				printf("\r\n");
		...
		if(file.fptr==file.fsize) 		//mp3文件读取完成，退出
		{
         ...
         //printf("%d, %d\n", MusicDialog.playindex, MusicDialog.music_file_num);
         //设置为列表循环播放
         ...
         //清除重绘制进度条的值
         InvalidateRect(MusicDialog.MUSIC_Hwnd, &rc_cli, TRUE);
         SendMessage(MusicDialog.TIME_Hwnd, SBM_SETVALUE, TRUE, 0); //设置位置值
         //清除歌词记数
         lyriccount=0;
         SAI_Play_Stop();   
         MP3FreeDecoder(Mp3Decoder);
...
//                  DrawText(hdc, wbuf, -1, &rc_MusicTimes, DT_SINGLELINE | DT_CENTER | DT_VCENTER);//绘制文字
                  ...
                  //更新进度条
                  SendMessage(MusicDialog.TIME_Hwnd, SBM_SETVALUE, TRUE, curtime*255/alltime);
                  InvalidateRect(MusicDialog.MUSIC_Hwnd, &rc_cli, FALSE);   
                  ...
                  if(lrc.flag == 1){
                     ...
                     if((lrc.oldtime <= lrc.curtime*100+100)&&(lrc.indexsize>7))
                     {
                        ...
                        SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC3),wbuf);
                        //显示第i-1行的歌词（前一行）
                        if(lyriccount>0)
                        {
                           x_mbstowcs_cp936(wbuf, (const char *)&ReadBuffer1[lrc.addr_tbl[lyriccount-1]-1], LYRIC_MAX_SIZE);
                           SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC2),wbuf);
                        }
                        else
                           SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC2),L" ");
                        //显示第i-2行的歌词（前两行）
                        if(lyriccount>0)
                        {
                           x_mbstowcs_cp936(wbuf, (const char *)&ReadBuffer1[lrc.addr_tbl[lyriccount-2]-1], LYRIC_MAX_SIZE);
                           SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC1),wbuf);
                        }
                        else
                           SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC1),L" ");
                        //显示第i+1行的歌词（后一行）   
                        if(lyriccount < lrc.indexsize-1)
                        {
                           x_mbstowcs_cp936(wbuf, (const char *)&ReadBuffer1[lrc.addr_tbl[lyriccount+1]-1], LYRIC_MAX_SIZE);
                           SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC4),wbuf);                    
                        }
                        else
                           SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC4),L" ");
                        //显示第i+2行的歌词（后二行）   
                        if(lyriccount < lrc.indexsize-2)
                        {
                           ...
                           SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC5),L" ");
                  ...
                  else
                  {
                     ...
                     SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC3),L"请在SDCard放入相应的歌词文件(*.lrc)");
                     SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC1),L" ");
                     SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC2),L" ");
                     SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC4),L" ");
                     SetWindowText(GetDlgItem(MusicDialog.MUSIC_Hwnd, eID_TEXTBOX_LRC5),L" ");
