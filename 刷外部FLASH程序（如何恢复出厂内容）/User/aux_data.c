...
FRESULT burn_file_sd2flash(Aux_Data_Typedef *dat,uint8_t file_num) 
{
    ...
    for(i=0;i<file_num;i++)
    {
      ...
      while(result == FR_OK) 
      {
        ...
        state = BSP_QSPI_Write(tempbuf,write_addr,256);  //拷贝数据到外部flash上         
        if(state != QSPI_OK)
        {
          BURN_ERROR("QSPI_Write ERROR");
      ...
      while(result == FR_OK) 
      {
        ...
        state = BSP_QSPI_Read(flash_buf,write_addr,bw);  //从FLASH中读取数据
        if(state != QSPI_OK)
        {
          BURN_ERROR("QSPI_Read ERROR");
          BURN_ERROR("loop=%d",loop);
...
FRESULT copy_file_sd2flash(char *src_path,char *dst_path) 
{
  ...
  //挂载flash
  //result = f_mount(&flash_fs,QSPIPath,1);
  
  BURN_INFO("正在格式化FLASH..."); 

  //格式化FLASH
  result = f_mkfs(QSPIPath,0,0,workBuffer, sizeof(workBuffer));							

  //重新挂载flash
  result = f_mount(NULL,QSPIPath,0);
  result = f_mount(&flash_fs,QSPIPath,0);
