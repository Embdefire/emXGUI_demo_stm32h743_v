...
//	vTaskList((char *)&CPU_RunInfo); //鑾峰彇浠诲姟杩愯鏃堕棿淇℃伅
...
static void MusicList_ReturnButton_OwnerDraw(DRAWITEM_HDR *ds) //缁樺埗涓�涓寜閽瑙?
{
  HWND hwnd;
  HDC hdc;
  RECT rc;
  ...
  //		SetBrushColor(hdc,MapRGB(hdc,150,200,250)); //璁剧疆濉厖鑹?BrushColor鐢ㄤ簬鎵�鏈塅ill绫诲瀷鐨勭粯鍥惧嚱鏁?
  ...
  //	FillRect(hdc,&rc); //鐢ㄧ煩褰㈠～鍏呰儗鏅?
  //	DrawRect(hdc,&rc); //鐢荤煩褰㈠妗?
  //  
  //  FillCircle(hdc,rc.x+rc.w/2,rc.x+rc.w/2,rc.w/2); //鐢ㄧ煩褰㈠～鍏呰儗鏅疐illCircle
  //	DrawCircle(hdc,rc.x+rc.w/2,rc.x+rc.w/2,rc.w/2); //鐢荤煩褰㈠妗?
...
static void MusicList_Button_OwnerDraw(DRAWITEM_HDR *ds) //缁樺埗涓�涓寜閽瑙?
{
	HWND hwnd;
	HDC hdc;
	RECT rc;
...
//        GUI_DEBUG("%s", p+4);
    ...
    case WM_DRAWITEM:
    {
      ...
      if(ds->ID == 0x2003)
        ...
        MusicList_Button_OwnerDraw(ds); //鎵ц鑷粯鍒舵寜閽?
      return TRUE;
